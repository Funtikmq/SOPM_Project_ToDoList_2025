...
const Task = ({
  ...
  const participantBadges = useMemo(() => {
    ...
    const ownerName =
      task?.ownerName || (isOwner ? user?.displayName : "") || ownerUsername;
  ...
  return (
    <div className="taskWrapper" data-task-id={task.id}>
      <div
        className="task taskRowCard taskRowClickable"
      ...
      >
        ...
        <div className="taskItem taskTitleCell">
          <div className="taskTitleRow">
            ...
            {isShared && (
              <span className="sharedIcon" aria-label="shared task">
                🤝
              </span>
            )}{" "}
          </div>
            ...
            >
              ...
              <div className="avatarStack">
                ...
                {participantBadges.length > 4 && (
                  <span className="collabAvatar more">
                    +{participantBadges.length - 4}
                  </span>
                )}
              </div>
        ...
        <div className="taskItem deadlineCell">
          <button
            className={`taskDeadline deadlineChip taskDeadlineBadge ${
              isNearDeadline(task.deadline) ? "deadlineWarning" : ""
            }`}
            onClick={(e) => {
              e.stopPropagation();
              if (!canEdit) return;
      ...
      <div
        className={`subtasksWrapper ${isExpanded ? "open" : ""}`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="taskExpand open">
          <div className="subtaskSection subtasksContainer">
            <div className="subtaskHeader">
              <div className="subtaskTitle">
                ...
                <span className="subtaskProgress">
                  {t ? t("progress") : "Progress"}: {progress.done}/
                  {progress.total}
                </span>
              </div>
            ...
            {subtasks.length === 0 ? (
              <div className="subtaskEmpty">
                {t ? t("noTasks") : "No subtasks yet"}
              </div>
            ) : (
                ...
                {subtasks.map((s) => (
                  <div key={s.id} className="subtaskRow">
                    <input
                      ...
                      disabled={!canEdit}
                    />
                    <span className={`subtaskText ${s.done ? "done" : ""}`}>
                      {s.title}
                    </span>
                    <button
