...
export const canEdit = (task, uid) => {
  ...
  return collab ? collab.role === ROLE_EDITOR || collab.role === ROLE_OWNER : false;
};
...
export const listenTasksForUser = (uid, callback) => {
  if (!uid) return () => {};
  const q = query(tasksCol, where("participants", "array-contains", uid), orderBy("updatedAt", "desc"));
  return onSnapshot(q, (snap) => {
    const list = snap.docs.map((d) => normalizeTask({ id: d.id, ...d.data() }, uid));
    callback(list);
  });
};
